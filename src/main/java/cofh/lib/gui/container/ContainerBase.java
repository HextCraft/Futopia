...
public abstract class ContainerBase extends Container {
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (stackInSlot.getMaxStackSize() <= 0) {
				slot.putStack(null);
			} else {
				slot.putStack(stackInSlot);
			}

			if (stackInSlot.getMaxStackSize() == stack.getMaxStackSize()) {
				return null;
			}
	...
	@Override
	public void putStackInSlot(int slotID, ItemStack stacks) {

		for (int i = 0; i < stacks.getMaxStackSize(); ++i) {
			putStackInSlot(i, stacks);
		}
	}
