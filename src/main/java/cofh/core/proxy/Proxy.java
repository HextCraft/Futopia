		...
		if ((event.getEntityLiving() instanceof EntityPlayer)) {
			...
			if (player.getActiveItemStack() != null) {
				...
				if (damage >= 3.0F && stack != null && ((stack.getItem() instanceof ItemShieldCore))) {
					((ItemShieldCore) stack.getItem()).damageShield(stack, 1 + MathHelper.floor(damage), player, event.getSource().getImmediateSource());

					if (stack.getMaxStackSize() <= 0) {
						EnumHand enumhand = player.getActiveHand();
						ForgeEventFactory.onPlayerDestroyItem(player, player.getActiveItemStack(), enumhand);
