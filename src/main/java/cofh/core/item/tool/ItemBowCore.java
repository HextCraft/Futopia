...
public class ItemBowCore extends ItemBow implements IEnchantableItem, IFOVUpdateItem {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		ActionResult<ItemStack> ret = net.minecraftforge.event.ForgeEventFactory.onArrowNock(player.getActiveItemStack(), world, player, hand, flag);
		if (ret != null) {
			return ret;
		}

		if (!player.capabilities.isCreativeMode && !flag) {
			return !flag ? new ActionResult<>(EnumActionResult.FAIL, player.getActiveItemStack()) : new ActionResult<>(EnumActionResult.PASS, player.getActiveItemStack());
		} else {
			...
			if (arrowStack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!flag) {
						...
						if (arrowStack.getMaxStackSize() == 0) {
