...
public class ContainerGeneratorBase extends Container {
	...
	public ContainerGeneratorBase(IInventory player, TileEntityGeneratorBase tile) {
		...
		for (int y = 0; y < 3; y++)
			for (int x = 0; x < 9; x++)
				addSlotToContainer(new Slot(player, x + y * 9 + 9, xs + (x * 18), ys + (y * 18) + 92)); // Player
																										// Inventory

		for (int k = 0; k < 9; k++)
			addSlotToContainer(new Slot(player, k, xs + (k * 18), ys + 83 + 67)); // Player
																					// Hotbar

		// addSlotToContainer(new Slot(player, 1, 100, 100));

		// addSlotToContainer(new SlotDefinition(SlotType.SLOT_SPECIFICITEM, new
