...
//        IntStream.range(0, 8).forEach(meta -> registerWithHandlers("oreFutopia" + StringUtils.capitalize(EnumMaterialType.values()[meta].getName()), new ItemStack(FTBlocks2.ores, 1, meta)));
//        IntStream.range(0, 8).forEach(meta -> registerWithHandlers("blockFutopia" + StringUtils.capitalize(EnumMaterialType.values()[meta].getName()), new ItemStack(FTBlocks2.storage, 1, meta)));
