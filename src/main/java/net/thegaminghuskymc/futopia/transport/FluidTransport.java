...
public class FluidTransport {
	...
	public static FluidActionResult tryEmptyOrFillContainer(ItemStack stack, IFluidHandler handler, int maxTransfer,
			EntityPlayer player) {
		FluidActionResult result = FluidUtil.tryEmptyContainer(stack, handler, maxTransfer, player, true);
		if (result.success) {
			return result;
			/*
			 * ItemStack remaining = result.result; if (remaining==null ||
			 * remaining==ItemStack.EMPTY) { player.setHeldItem(hand,
			 * ItemStack.EMPTY); } else { player.setHeldItem(hand, remaining); }
			 * System.out.println("SUCCESS: "+remaining); return true;
			 */
		} else {
			...
			return resultFill;
			/*
			 * if (resultFill.success) { ItemStack remaining =
			 * resultFill.result; if (remaining==null ||
			 * remaining==ItemStack.EMPTY) { player.setHeldItem(hand,
			 * ItemStack.EMPTY); } else { player.setHeldItem(hand, remaining); }
			 * System.out.println("SUCCESS-Empty: "+remaining); return true; }
			 * System.out.println("UNSUCCESS-Empty"); player.swingArm(hand);
			 * return true;
			 */
		}
	}
	...
	public static boolean handleRightClickFluidStorage(EntityPlayer player, EnumHand hand, IFluidHandler tank,
		...
		if (result.isSuccess()) {
			// System.out.println("Successful fluid interaction! Result is:
