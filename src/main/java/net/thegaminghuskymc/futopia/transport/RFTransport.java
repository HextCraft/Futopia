package net.thegaminghuskymc.futopia.transport;
...
public class RFTransport {
	...
	public static void diffuse(World world, BlockPos pos, IEnergyStorage host) {
		...
		for (IEnergyStorage consumer : consumers) {
			// int totalEnergy = host.getEnergyStored() +
			// consumer.getEnergyStored();
			// float energyRatio = host.getMaxEnergyStored() /
			...
			if (sourceLevel > destLevel) {
				 ...
				 * 
				 * totalEnergy: 3900 energyRatio: 1.0f sourceLevel: 0.55f
				 * destLevel: 0.425f availableQuanta: 500 / 2 - 1 = 249
				 * 
				 */

				/*
				 * Transmission data: 0 blocks: 775.0 1 block : 393.6 2 blocks:
				 * 260.8 3 blocks: 196.6 4 blocks: 157.8 5 blocks: 131.8
				 */

				int availableQuanta = ((int) ((sourceLevel - destLevel) * host.getMaxEnergyStored()) / 2); // trunc
																											// leaves
																											// fractional
																											// power
																											// differences
				...
				// debugString.append("(");
				// debugString.append((int)(sourceLevel*100));
				// debugString.append("%) -> ");
				// debugString.append(consumer.getEnergyStored());
				// debugString.append("(");
				// debugString.append((int)(destLevel*100));
				// debugString.append("%) :");

				// debugString.append(" req=");
				// debugString.append(availableQuanta);
	...
	public static IEnergyStorage getStorage(World world, BlockPos pos, EnumFacing side) {
		...
		if (te == null)
			return NULL_ENERGY; // If it can't Capabilities, it can't
								// IEnergyStorage
		if (te.hasCapability(CapabilityEnergy.ENERGY, side)) {
			return te.getCapability(CapabilityEnergy.ENERGY, side);
		} else {
			return NULL_ENERGY; // Some day we might support other APIs. Not
								// today.
		}
	}
	 ...
	 * 
	 * @param t
	 *            the List to reorder. Do not pass this method an immutable
	 *            list!!!
	 */
