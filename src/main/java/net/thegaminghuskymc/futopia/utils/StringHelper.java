...
public final class StringHelper {
	...
	public static String titleCase(String input) {
		return (new StringBuilder()).append(input.substring(0, 1).toUpperCase(Locale.US)).append(input.substring(1))
				.toString();
	}
	...
	public static String getScaledNumber(long number) {
		if (number >= 0x3b9aca00L)
			return (new StringBuilder()).append(number / 0x3b9aca00L).append(".")
					.append((number % 0x3b9aca00L) / 0x989680L).append("G").toString();
		if (number >= 0xf4240L)
			return (new StringBuilder()).append(number / 0xf4240L).append(".").append((number % 0xf4240L) / 10000L)
					.append("M").toString();
		if (number >= 1000L)
			return (new StringBuilder()).append(number / 1000L).append(".").append((number % 1000L) / 10L).append("k")
					.toString();
		else
